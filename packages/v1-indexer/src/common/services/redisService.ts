...
export const getInformationPerVAMM = async (
  ...
  const key = `${getEnvironmentV1()}_${information}_${chainId}_${vammAddress}`;
...
export const getInformationPerMarginEngine = async (
  ...
  const redisClient = getRedisClient();
  const key = `${getEnvironmentV1()}_${information}_${chainId}_${marginEngineAddress}`;
  const value = await redisClient.get(key);
...
export const getInformationPerChain = async (
  ...
  const redisClient = getRedisClient();
  const key = `${getEnvironmentV1()}_${information}_${chainId}`;
  const value = await redisClient.get(key);
