...
jest.mock('@voltz-protocol/commons-v2', () => ({
  ...
  convertLowercaseString: jest.fn(() => {}),
  insertCollateralUpdateEvent: jest.fn(() => {}),
}));
...
describe('Collateral Update Indexer unit tests', () => {
  it('new event', async () => {
    ...
    (getTokenDetails as jest.Mock).mockReturnValue({
      tokenDescaler: (value: BigNumber) => 100,
    });

    // Mock pullCollateralUpdateEvent
    (pullCollateralUpdateEvent as jest.Mock).mockResolvedValueOnce(null);

    // Mock insertCollateralUpdateEvent
    (insertCollateralUpdateEvent as jest.Mock).mockResolvedValueOnce(query());
  ...
  it('existing event', async () => {
    ...
    (getTokenDetails as jest.Mock).mockReturnValue({
      tokenDescaler: (value: BigNumber) => 0,
    });

    (convertLowercaseString as jest.Mock).mockReturnValueOnce('');

    (insertCollateralUpdateEvent as jest.Mock).mockReturnValueOnce(null);
