 ...
 * @param sandbox - If sandbox then it uses staging env. to check the TOS Signature, otherwise uses the production one
 * @param referralCode - The service url
 * */
export const checkForTOSSignature = async ({
  ...
  const signatureData = await getSignature(signerAddress, serviceUrl);
