...
describe('checkForRiskyWallet', () => {
  it('does not throw an error when wallet is not risky', async () => {
    await expect(checkForRiskyWallet('0x1234567890')).resolves.not.toThrow();
  });

  it('throws an error when wallet is risky', async () => {
    (isWalletRisky as jest.Mock).mockReturnValue(true);
    await expect(checkForRiskyWallet('0x1234567890')).rejects.toThrow(
