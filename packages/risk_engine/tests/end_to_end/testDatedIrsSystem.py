...
from packages.risk_engine.src.instruments.dated_irs.marketManager import MarketManager
from packages.risk_engine.src.constants import DAY_IN_SECONDS, MONTH_IN_SECONDS
from packages.risk_engine.src.exchanges.vamm.datedIRSVAMMExchange import DatedIRSVAMMExchange
from packages.risk_engine.tests.end_to_end.helpers import get_rate_given_apy
...
class Test(unittest.TestCase):
    def get_observations(self, relative_timestamp):
        ...
        rate_10 = get_rate_given_apy(rate_start=rate_0, time=10 / 365, apy_in_between=0.06)

        t_20 = 20 * DAY_IN_SECONDS
        rate_20 = get_rate_given_apy(rate_start=rate_10, time=10 / 365, apy_in_between=0.06)

        t_30 = 30 * DAY_IN_SECONDS
        rate_30 = get_rate_given_apy(rate_start=rate_20, time=10 / 365, apy_in_between=0.06)
    ...
    def setUp(self):
        ...
        self.observations = self.get_observations(relative_timestamp=self.block.timestamp)
        self.oracle = Oracle(block=self.block, initial_observations=[self.observations[0]])
        ...
        self.market_irs_usdc: DatedIRSMarket = DatedIRSMarket(block=self.block, market_id="market_irs_usdc")

        self.market_irs_usdc.set_collateral_module(collateral_module=self.collateral_module)
        self.market_irs_usdc.set_liquidation_module(liquidation_module=self.liquidation_module)
        ...
        # Set up account
        self.alice = self.account_manager.create_account(account_id="alice", base_token="USDC")

        self.bob = self.account_manager.create_account(account_id="bob", base_token="USDC")
    ...
    def test(self):
        ...
        self.collateral_module.deposit_collateral(account_id=self.alice.account_id, amount=200)
        ...
        balance = self.collateral_module.get_account_collateral_balance(account_id="alice")
        self.assertAlmostEqual(balance, 200 - 0.452054794520548)

        # Alice: Check orders
        bases = self.alice.get_annualized_filled_and_unfilled_orders()
        self.assertAlmostEqual(bases[0]["unfilled_long"], 500 * 30 / 365 * self.observations[0][1])
        self.assertAlmostEqual(bases[0]["unfilled_short"], -500 * 30 / 365 * self.observations[0][1])
        ...
        self.collateral_module.deposit_collateral(account_id=self.bob.account_id, amount=200)
        ...
        # Bob: Check fees paid
        balance = self.collateral_module.get_account_collateral_balance(account_id="bob")
        ...
        self.assertAlmostEqual(bases[0]["filled"], 200 * 20 / 365 * self.observations[1][1])
        ...
        # Alice: Check orders
        bases = self.alice.get_annualized_filled_and_unfilled_orders()
        self.assertAlmostEqual(bases[0]["unfilled_long"], 700 * 10 / 365 * self.observations[2][1])
        self.assertAlmostEqual(bases[0]["unfilled_short"], -300 * 10 / 365 * self.observations[2][1])
        self.assertAlmostEqual(bases[0]["filled"], -200 * 10 / 365 * self.observations[2][1])
        ...
        # Alice: Settle
        self.market_irs_usdc.settle(maturity=self.pool_irs_usdc_maturity, account_id="alice")

        # Bob: Settle
        self.market_irs_usdc.settle(maturity=self.pool_irs_usdc_maturity, account_id="bob")
