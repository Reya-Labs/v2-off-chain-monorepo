...
class DatedIRSMarket(BaseMarket):
    ...
    def get_unrealized_pnl_in_quote(self, maturity, account_id):
        base, quote = self.get_account_filled_balances(maturity=maturity, account_id=account_id)

        time_delta = max(0, (maturity - self.block.timestamp) / YEAR_IN_SECONDS)
        unwind_quote = (
            base * self._oracle.latest() * (self.aggregate_gwap(maturity=maturity) * time_delta + 1)
        )
