...
class StatefulTakerPositionTransformDoFn(beam.DoFn):
    ...
    '''
        ...
        should be parallelised for each positionid
    '''

    TAKER_POSITION_STATE = BagStateSpec('taker_position', TupleCoder((StrUtf8Coder, TimestampCoder, BigIntegerCoder)))

    def process(self, initiateTakerOrderEvent, cached_taker_position_state=beam.DoFn.StateParam(TAKER_POSITION_STATE)):

        fees_paid_to_initiate_taker_order = initiateTakerOrderEvent['feesPaid']
        current_taker_order_event_timestamp = initiateTakerOrderEvent['timestamp']
        position_id = initiateTakerOrderEvent['positionId']
