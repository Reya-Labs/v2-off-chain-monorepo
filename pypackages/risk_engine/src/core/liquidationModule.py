...
class LiquidationModule:
    def __init__(
        self, im_multiplier: float, liquidator_reward_proportion_of_im_delta: float
    ):
        self._risk_mapping = None
        self._im_multiplier: float = im_multiplier
        self.liquidator_reward_proportion_of_im_delta: float = (
            liquidator_reward_proportion_of_im_delta
        )
    ...
    def is_im_satisfied(
        ...
        account_net_worth = collateral_module.get_account_total_value(
            account_id=account_id, account_manager=account_manager
        )
        IMR, _ = self.get_account_margin_requirements(
            account_id=account_id, account_manager=account_manager
        )

        return account_net_worth >= IMR

    def is_account_liquidatable(
        self,
        account_id,
        account_manager: AccountManager,
        collateral_module: CollateralModule,
    ):
        account_value = collateral_module.get_account_total_value(
            account_id=account_id, account_manager=account_manager
        )
        IMR, LMR = self.get_account_margin_requirements(
            account_id=account_id, account_manager=account_manager
        )
    ...
    def liquidate_account(
        ...
        if not liquidator.get_base_token() == liquidation_token_type:
            raise Exception(
                "liquidation engine: liquidator account is based on different token"
            )

        (
            is_account_liquidatable,
            im_pre_account_closing,
            _,
        ) = self.is_account_liquidatable(
        ...
        im_post_account_closing, _ = self.get_account_margin_requirements(
            account_id=liquidated_account_id, account_manager=account_manager
        )
        ...
        liquidator_reward_amount = (
            delta_im * self.liquidator_reward_proportion_of_im_delta
        )
    ...
    def get_account_margin_requirements(
        self, account_id, account_manager: AccountManager
    ):
        ...
        annualized_filled_and_unfilled_orders = (
            account.get_annualized_filled_and_unfilled_orders()
        )

        worst_case_filled_orders = []

        for order in annualized_filled_and_unfilled_orders:
            risk_parameter = self.get_risk_parameter(
                market_id=order["market_id"], maturity=order["maturity"]
            )
    ...
    def get_worst_case_cashflows_post_up_and_down_shocks(
        self, annualized_worst_filled_up_and_down_notionals
    ):
        ...
        for order in annualized_worst_filled_up_and_down_notionals:
            ...
            worst_case_cashflow_down_shock += (
                order["worst_filled_down"] * risk_parameter
            )
